...
export function demoCodePlugin(snippets: SnippetMap): Plugin {
  return {
    ...
    transformIndexHtml(html: string) {
      return html.replace(/<code([^>]*data-code="([^"]+)"[^>]*)><\/code>/g, (_match, attrs: string, key: string) => {
        const source = snippets[key];
        if (!source) return `<code${attrs}></code>`;
        const language = attrs.includes('data-language="shell"') ? "shell" : attrs.includes('data-language="html"') ? "html" : "typescript";
